...
import renderConfirmEmailTemplate from './templates/confirmEmail'
...
export async function sendVerificationEmail(event: H3Event, email: string) {
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      html: renderConfirmEmailTemplate({
        callback: `${getRequestURL(event).origin}/confirmEmail/${emailConfirmation.id}`,
      }),
    })
  } catch (e) {
    console.error(e)
